...
// Maps the contract's custom revert errors (see gleeABI's "error" entries) to messages a
// wallet holder can actually act on. Without this, a simulation that already knows a call
// will revert was being silently discarded — the button stayed clickable, the wallet was
// left to discover the revert on its own, and different wallets surfaced that differently
// (Rainbow: "simulation failed" but lets you proceed anyway; Coinbase/Base Wallet: hangs).
const MINT_ERROR_MESSAGES: Record<string, string> = {
  InvalidMerkleProof: "This wallet's whitelist proof couldn't be verified. Try reloading the page.",
  WLAllocationExceeded: "That quantity would exceed your whitelist allocation.",
  WLWindowClosed: "The whitelist window has closed — switch to public mint.",
  PublicCapExceeded: "That quantity would exceed the public per-wallet limit.",
  MaxSupplyReached: "The collection is sold out.",
  WrongPrice: "The mint price just changed — refresh the page and try again.",
  NotOpen: "Minting isn't open yet.",
  ConfigLocked: "Whitelist configuration is locked.",
  MorePlease: "Enter a quantity of at least 1.",
  RefundFailed: "The refund for excess ETH failed — contact support with your transaction hash.",
};

function decodeMintError(error: unknown): string | null {
  let current = error as { message?: string; shortMessage?: string; cause?: unknown } | undefined;
  for (let depth = 0; depth < 6 && current; depth += 1) {
    const haystack = `${current.shortMessage ?? ""} ${current.message ?? ""}`;
    for (const [name, message] of Object.entries(MINT_ERROR_MESSAGES)) {
...
export default function MintCanvas({ fallbackPriceEth = 0.003, onMintSuccess }: MintCanvasProps) {
  ...
  const isOnWhitelist = Boolean(whitelistStatus?.eligible);
  const hasWlRemaining = wlRemainingForMe !== undefined && (wlRemainingForMe as bigint) > BigInt(0);
  const isEligibleForWl = isOnWhitelist && hasWlRemaining;
  const canUseWhitelist = isWlWindowActive && isEligibleForWl;
  ...
  const mintErrorMessage = !hasInsufficientFundsError ? decodeMintError(simulateError) : null;
  ...
  const handleMint = () => {
    if (!totalPriceWei || simulateError) return;
    if (mode === "whitelist" && whitelistStatus) {
      writeContract({
        ...contract,
        functionName: "mintWhitelist",
  ...
  return (
    <div>
      ...
      {!isConnected && <p className="mt-3 text-sm text-[var(--accent)]">Connect your wallet to mint.</p>}
      {isConnected && isWlWindowActive && !isOnWhitelist && (
        <p className="mt-3 text-sm text-[var(--foreground-muted)]">
          This wallet isn&apos;t on the whitelist — public mint is still available if there&apos;s unreserved supply left.
        </p>
      )}
      ...
      {hasInsufficientFundsError && <p className="mt-3 text-sm text-[#c17a72]">Insufficient funds for gas and mint price.</p>}
      {!hasInsufficientFundsError && mintErrorMessage && <p className="mt-3 text-sm text-[#c17a72]">{mintErrorMessage}</p>}
    </div>
  );
}